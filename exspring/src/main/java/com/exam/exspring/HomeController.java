...
public class HomeController {
	...
	//@RequestMapping : 웹 요청을 받아서 실행되는 코드를 담고 있는 메서도
	//path 또는 value 값으로 ,요청 경로(주소) 설정
	//method 값으로 요청 방식 설정
	@RequestMapping(value = "/home.do", method = RequestMethod.GET)
    //locale:현재 지역정보를 받아옴,model: jsp와 공유할 데이터
	//@RequestParam("파라미터이름") String id-->요청 파라미터값을 넣을 변수 id에 "파라미터이름"으로 값을 변수에 넣어준다.
	public String home(Locale locale
			, @RequestParam("myId")int id //myId라는 요청 파라미터값을 id 변수에 저장(자동형변환)
			...
			//Map은 원래 자바에 있는거고,Model과 ModepMap은 스프링에서 제공
			//key-value 형태로 데이터를 저장하면 JSP에서 ${key}표현으로 사용가능
			, Map map
			, ModelMap modelMap
			, Model model) {
		...
		logger.info("vo.myId:{}.",vo.getMyName());
...
//		이렇게 포워딩된다
		...
		//스프링을 그 문자열을 뷰이름으로 인식하고 처리(ViewResolver를 사용하여 처리)
	}
	...
	public ModelAndView aaa() {
		...
		//mav.setView(new InternalResourceView("/exspring/src/main/webapp/WEB-INF/views/test.jsp"));//화면을 출력할 뷰 객체 설정
		...
		//mav.setViewName("/WEB-INF/views/test.jsp"); 이렇게 알려주면 안됨,접두어와 확장자를 다 지우고 알려줘야함
		mav.setViewName("test");// 뷰 객체 대신 뷰 이름으로 설정가능
		
		return mav;
	}
	
	//(2)뷰 정보만(객체,이름)만 반환하는 방법
	...
	public String bbb(Map<String,Object> map) {
		...
		//return new InternalResourceView("/WEB-INF/views/test.jsp");//화면을 출력할 뷰 객체 설정
		//mav.setView(new InternalResourceView("/WEB-INF/views/test.jsp"));//화면을 출력할 뷰 객체 설정
		//InternalResourceView 객체는 지정한 경로의 파일로 이동(forward)시키는 뷰
		//mav.setViewName("/WEB-INF/views/test.jsp"); 이렇게 알려주면 안됨,접두어와 확장자를 다 지우고 알려줘야함
		//mav.setViewName("test");// 뷰 객체 대신 뷰 이름으로 설정가능
	...
	//(3)모델 정보만 반환하는 방법
	//반환한 객체를 클래스명(MyNo)의 첫글저만 소문자로 변경한 이름(myVo)으로 모델에 저장(추가)
	//모델에 저장되는 이름을 지정하고 싶으면 ,@ModelAttibute("이름")을 사용
	//뷰정보를 반환하지 않으면,(컨텍스트패스를 제외한)요청주소에서 첫/와 마지막 확장자를 제거한 문자열을 뷰 이름으로 사용 
	//http://localhost:8000/exspring까지가 컨텍스트패스고 확장자는 .do이므로
	//http://localhost:8000/exspring/test3.do-->"test3" 를 뷰이름으로 씀
	//그렇기에 test.jsp로 보내기 위해 test3.do로 되있던 경로를 test.do로 변경하였음
